...
theorem mul_inv_cancel (a : G) : a * a⁻¹ = 1 := by
  calc a * a⁻¹
    ...
    _ = (a⁻¹)⁻¹ * (a⁻¹ * a) * a⁻¹ := by rw[mul_assoc _ a⁻¹ a]
    _ = (a⁻¹)⁻¹ * (1 * a⁻¹) := by rw [inv_mul_cancel, mul_assoc]
    _ = (a⁻¹)⁻¹ * a⁻¹ := by rw [one_mul] -- use one_mul not mul_one to avoid circular reasoning
    _ = 1 := by rw[inv_mul_cancel]

theorem mul_one (a : G) : a * 1 = a := by
  have h1 := by apply inv_mul_cancel a
  rw [← h1, ← mul_assoc]
  rw [mul_inv_cancel, one_mul]

theorem mul_inv_rev (a b : G) : (a * b)⁻¹ = b⁻¹ * a⁻¹ := by
  have h1 := by apply inv_mul_cancel (a * b)
  have h2: b⁻¹ * a⁻¹ = (a * b)⁻¹ :=
    calc b⁻¹ * a⁻¹
      _ = 1 * (b⁻¹ * a⁻¹) := by rw [one_mul]
      _ = (a * b)⁻¹ * (a * b) * (b⁻¹ * a⁻¹) := by rw[h1]
      _ = (a * b)⁻¹ * (a * b * (b⁻¹ * a⁻¹)) := by rw[mul_assoc]
      _ = (a * b)⁻¹ * (a * (b * b⁻¹) * a⁻¹) := by rw[← mul_assoc _ b⁻¹ _, mul_assoc a b _]
      _ = (a * b)⁻¹ * (a * 1 * a⁻¹) := by rw [mul_inv_cancel]
      _ = (a * b)⁻¹ * (a * a⁻¹) := by rw [mul_one]
      _ = (a * b)⁻¹ * 1 := by rw [mul_inv_cancel]
      _ = (a * b)⁻¹ := by rw [mul_one]
  symm; apply h2
